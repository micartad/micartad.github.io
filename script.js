...
function renderMenuItem(item) {
    ...
    const mensajeWhatsApp = encodeURIComponent(
        `¡Hola ${CONFIG.RESTAURANT_INFO.name}! Quiero pedir:\n\n` +
        `• ${item.nombre} - $${item.precio}\n\n` +
        `💰 *Total: $${item.precio}*\n\n` +
        `👤 *Mis datos:*\n[Nombre]\n[Dirección de entrega]\n[Teléfono]\n\n` +
        `📍 ${CONFIG.RESTAURANT_INFO.address}\n⏰ ${CONFIG.RESTAURANT_INFO.schedule}`
    );
    
    const whatsappUrl = `https://wa.me/${CONFIG.RESTAURANT_INFO.whatsapp.number}?text=${mensajeWhatsApp}`;
    ...
    return `
        <article class="menu-item">
            ...
            <div class="item-content">
                ...
                <a href="${whatsappUrl}" 
                   target="_blank" 
                   class="whatsapp-btn-directo"
                   onclick="trackWhatsAppOrder('${item.nombre.replace(/'/g, "\\'")}', ${item.precio})">
...
// Agregar estilos para el botón de WhatsApp
function agregarEstilosWhatsApp() {
    const estilos = `
        /* Botón WhatsApp directo */
        .whatsapp-btn-directo {
            display: inline-flex;
            align-items: center;
...
// Función para trackear pedidos por WhatsApp (opcional)
function trackWhatsAppOrder(producto, precio) {
    console.log(`Pedido por WhatsApp: ${producto} - $${precio}`);
...
// Inicializar
function init() {
    // Agregar estilos del botón WhatsApp
    agregarEstilosWhatsApp();
